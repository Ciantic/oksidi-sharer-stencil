...
@Component({
    tag: "oksidi-sharer",
    styleUrl: "oksidi-sharer.scss",
    shadow: true,
})
export class OksidiSharer {
    @Prop() locale = getLocale(document.documentElement.lang);
    @Prop() shareUrl: string = window.location.href;
    @Prop() shareTitle: string = "";
    @Prop() useFacebook = true;
    @Prop() useTwitter = true;
    @Prop() useWhatsapp = true;
    @Prop() useLink = true;
    @Prop() useEmail = true;
    @Prop() useLinkedin = false;
    @Prop() textShare: string = TRANSLATIONS.textShare[this.locale];
    @Prop() textCopy: string = TRANSLATIONS.textCopy[this.locale];
    @Prop() css = "";

    @State() isOpen = false;
    @State() isOpenAnim = false;
    @State() isOpenAnim2 = false;
    @State() showCopyToClipboardTooltip = false;

    private onClickToggle(e: MouseEvent) {
        e.preventDefault();
        this.isOpen = !this.isOpen;
        if (this.isOpen) {
            this.isOpenAnim = true;
            setTimeout(() => {
                this.isOpenAnim2 = this.isOpenAnim;
            }, 70);
        } else {
            this.isOpenAnim2 = false;
            setTimeout(() => {
                this.isOpenAnim = this.isOpen;
            }, 250);
        }
    }

    private onClickCopyToClipboard(e: MouseEvent) {
        e.preventDefault();

        copyToClipboard(this.shareUrl, e.currentTarget as HTMLElement);

        this.showCopyToClipboardTooltip = true;
        setTimeout(() => {
            this.showCopyToClipboardTooltip = false;
        }, 1500);
    }

    render() {
        let urlUE = encodeURIComponent(this.shareUrl);
        let titleUE = encodeURIComponent(this.shareTitle);
        let facebookSharingUrl = `https://www.facebook.com/sharer.php?u=${urlUE}`;
        let twitterSharingUrl = `https://twitter.com/intent/tweet?text=${titleUE}&url=${urlUE}`;
        let linkedInSharingUrl = `https://www.linkedin.com/shareArticle?mini=true&url=${urlUE}&title=${titleUE}&summary=&source=LinkedIn`;
        let whatsAppSharingUrl = `whatsapp://send?text=${urlUE}`;
        let mailSharingUrl = `mailto:?subject=${titleUE}&body=${urlUE}`;

        return (
            <div class="sharer">
                {this.css && <style>{this.css}</style>}
                <a href="#share" class="opener" onClick={this.onClickToggle.bind(this)}>
                    {!this.isOpen || !this.isOpenAnim ? (
                    ...
                    <span class="title">{this.textShare}</span>
                </a>
                {this.isOpenAnim && (
                    <ul class={`choices ${this.isOpenAnim2 ? "shown" : "hidden"}`}>
                        {this.useFacebook && (
                            <li>
                        ...
                        {this.useLink && (
                            <li>
                                <a
                                    ...
                                    onClick={this.onClickCopyToClipboard.bind(this)}
                                    target="_blank"
                                >
                                    {this.showCopyToClipboardTooltip && (
                                        <div class="copy-tip">{this.textCopy}</div>
                                    )}
